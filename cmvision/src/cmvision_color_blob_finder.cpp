...
bool CMVisionColorBlobFinder::initialize(ros::NodeHandle &node_handle)
{
	...
	cloud_sub_ = node_handle.subscribe("/stereo/points2", 1, &CMVisionColorBlobFinder::cloudCB, this);	//AS

	// Advertise our blobs
	blob_publisher_ = node_handle.advertise<cmvision::Blobs> ("blobs", 1);
	cloud_pub_ = node_handle.advertise<pointcloudT> ("flags", 1);
...
//added by AS
void CMVisionColorBlobFinder::cloudCB(const sensor_msgs::PointCloud2ConstPtr& msg) {
  mutex_.lock();
  fromROSMsg(*msg, pointcloud_);
  //ROS_INFO("cloud\n");	
	mutex_.unlock();
}
...
void CMVisionColorBlobFinder::imageCB(const sensor_msgs::ImageConstPtr& msg)
{
  ...
  if (blob_count_ < blob_message_.blobs.size())
	...
	mutex_.lock();
	if (blob_count_ > 0) {  
  //create a cloud for publishing
  	pcl::PointCloud<pcl::PointXYZ> cloud_out;
  	cloud_out.width = pointcloud_.width;
  	cloud_out.height = pointcloud_.height;
  	cloud_out.points.resize (cloud_out.width * cloud_out.height);
  
  	int top_, bottom_, left_, right_;
  	top_ = blob_message_.blobs[0].top;
  	//bottom_ =  blob_message_.blobs[0].bottom;
  	bottom_ = cloud_out.height;//extrapolate the flag
  	left_ = blob_message_.blobs[0].left;
  	right_ = blob_message_.blobs[0].right;
      
  	int i = 0;
  	//check the cloud_msg for cloudpoints around the blob and fill it in the created cloud
  	for (int u=top_; u<=bottom_; ++u) {
 	  	for (int v=left_; v<=right_; ++v) {
    	  i = (u*width_) + v;
      	if (isnan (pointcloud_.points[i].x) || isnan (pointcloud_.points[i].y) || isnan (pointcloud_.points[i].z)) 
        	continue;
      	//assign cloud around the blob to cloud_out for publishing  
      	cloud_out.points[i].x = pointcloud_.points[i].x;
      	cloud_out.points[i].y = pointcloud_.points[i].y;
      	cloud_out.points[i].z = pointcloud_.points[i].z;
      	//ROS_INFO("%f, %f, %f\n", cloud_out.points[i].x, cloud_out.points[i].y, cloud_out.points[i].z);      
    	}
  	}  
  	cloud_out.header.stamp = ros::Time::now();
  	cloud_out.header.frame_id = "/flag_cloud_frame";
